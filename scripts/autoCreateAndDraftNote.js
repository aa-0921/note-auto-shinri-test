...
(async () => {
  await runWithCore(async ({ core, wantsBackground }) => {
    ...
    const articleConditionsLines = [
      ...
      '本文は次の簡潔な構成で作成してください。',
      '1) 問題文（1〜2行）',
      '2) 科学的解説セクション（2〜3行の解説文）',
      '選択肢の直前に以下の固定的な解説を必ず挿入してください：「嫉妬心は心理学的には『所有権の脅威に対する防御反応』として研究されています。脳科学では、前頭前野で感情コントロールが働き、扁桃体で脅威を察知するプロセスが関わっていることが分かっています。色の選択は、私たちの無意識の感情状態を反映するものとして注目されています。」',
      '3) 選択肢（A/B/C/Dの4択、各1行）',
      '選択肢の直前に、次のガイド文を1行で必ず表示してください：「深呼吸して、A〜Dの中から心のままにひとつ選んでください✨」',
      '問題文には必ず「あなたが感じる嫉妬心を色に例えると、どの色が近いと思いますか？」のような明確な選択基準を含めてください。',
